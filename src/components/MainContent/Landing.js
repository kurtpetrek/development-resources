...
class Landing extends Component {
  ...
  componentDidMount() {
    ...
    window.onresize = event => {
      if (
        ((window.innerHeight % 150 > 0 && window.innerHeight % 150 < 10) ||
          (window.innerWidth % 50 > 0 && window.innerWidth % 50 < 10)) &&
        pos < height
      ) {
  ...
  handleScroll = () => {
    ...
    if (pos < height) {
      ...
      html.style.transform =
        "translate(" + Math.floor(pos / 8) + "%," + Math.floor(pos / 5) + "%)";
      css.style.transform =
        "translate(" +
        Math.floor(pos / -8) +
        "%," +
  ...
  handleCodeCanvas = () => {
    ...
    this.codeWorkAnm.putPixel = () => {
      ...
      this.codeWorkAnm.codeCtx.fillRect(
        0,
        0,
        this.codeWorkAnm.codeCanvas.width,
        this.codeWorkAnm.codeCanvas.height
      );
      ...
      for (var i = 0; i < this.codeWorkAnm.charDrops.length; i++) {
        var txt = this.codeWorkAnm.floatChar[
          Math.floor(Math.random() * this.codeWorkAnm.floatChar.length)
        ];
        this.codeWorkAnm.codeCtx.fillText(
          txt,
          i * this.codeWorkAnm.font_size,
          this.codeWorkAnm.charDrops[i] * this.codeWorkAnm.font_size
        );
        if (
          this.codeWorkAnm.charDrops[i] * this.codeWorkAnm.font_size >
            this.codeWorkAnm.codeCanvas.height &&
